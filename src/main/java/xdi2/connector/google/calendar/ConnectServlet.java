package xdi2.connector.google.calendar;
...
public class ConnectServlet extends HttpServlet implements HttpRequestHandler {
	...
	private static final long serialVersionUID = -4587826453722258850L;
	...
	static {
		...
		sampleEndpoint = "/xdi/google-calendar"; 
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("Request Access Token!".equals(request.getParameter("submit"))) {
			...
			try {
				...
				response.sendRedirect(this.getGoogleCalendarApi().startOAuth(request, null, userXri));
				return;
			} catch (Exception ex) {
		...
		if ("Revoke Access Token!".equals(request.getParameter("submit"))) {
			...
			try {
				...
				this.getGoogleCalendarApi().revokeAccessToken(accessToken);
		...
		if (request.getParameter("code") != null) {
			...
			try {
				...
				this.getGoogleCalendarApi().checkState(request, userXri);

				String accessToken = this.getGoogleCalendarApi().exchangeCodeForAccessToken(request);
		...
		request.getRequestDispatcher("/GoogleCalendarConnector.jsp").forward(request, response);
	}
